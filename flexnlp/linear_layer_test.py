...
from test_tool import tool as tool
from converter import Converter as cvtr

class linear_layer_test:
  ...
  def produce_ly_test_data(self):
    ...
    # -------------------------
    for i in range(self.num_ts):
      inp_ts = inp_init[self.num_v_in*16*i : self.num_v_in*16*(i+1)]
    ...
    # --------------------------
    wgt_q, bias_wgt = self.tl.get_adpfloat_bias(wgt_init)
    inp_q, bias_inp = self.tl.get_adpfloat_bias(inp_init)
    bias_q, bias_b = self.tl.get_adpfloat_bias(bias_init)

    print('\t*** performed weight matrix tiliing for PEs***\n')
    ...
    self.produce_data_lib_ly(param, './test/ly_data_lib.json')
  
  def produce_data_lib_ly(self, param, out_path):
    """
...
if __name__ == '__main__':
  ...
  test = linear_layer_test(num_v_in, num_v_out, num_ts, is_bias)
  test.run()
  test.clean_up()