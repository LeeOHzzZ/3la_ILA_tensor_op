...
class attention_layer(FlexASRBaseDriver):
  ...
  def _collect_data(self):
    ...
    self.encoder_data, self.adpbias_enc = self.tl.get_adpfloat_bias(self.encoder_data, self.ADPTFLOAT_N_BITS, self.ADPTFLOAT_N_EXP, self.ADPTBIAS)
    self.decoder_data, self.adpbias_dec = self.tl.get_adpfloat_bias(self.decoder_data, self.ADPTFLOAT_N_BITS, self.ADPTFLOAT_N_EXP, self.ADPTBIAS)
    self.adpbias_enc += self.ADPTFLOAT_OFFSET
    self.adpbias_dec += self.ADPTFLOAT_OFFSET
  ...
  def _get_ref_result(self):
    ...
    self.adpbias_softmax = 2
    self.adpbias_out = self.tl.get_adpfloat_bias(ref_cxt_vector, self.ADPTFLOAT_N_BITS, self.ADPTFLOAT_N_EXP, self.ADPTBIAS)[1] + self.ADPTFLOAT_OFFSET
